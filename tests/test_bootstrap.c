...
DECLARE_TEST(test_bootstrap_prefers_cli_path)
{
    ...
    char message[128];
    ASSERT_TRUE(app_bootstrap_decide_tree_source(&options, "assets/example_tree.json", &decision, message,
                                                 sizeof(message)));
...
DECLARE_TEST(test_bootstrap_handles_missing_sample)
{
    ...
    AppStartupDecision decision;
    char message[128];
    ASSERT_TRUE(app_bootstrap_decide_tree_source(&options, NULL, &decision, message, sizeof(message)));
    ASSERT_EQ(decision.source, APP_STARTUP_SOURCE_PLACEHOLDER);
    ASSERT_TRUE(strstr(message, "placeholder") != NULL);
}
...
DECLARE_TEST(test_bootstrap_rejects_null_inputs)
{
    AppStartupDecision decision;
    char message[64];
    ASSERT_FALSE(app_bootstrap_decide_tree_source(NULL, NULL, &decision, message, sizeof(message)));
