                        ...
                        const struct RenderConfig *config, const struct Camera3D *camera)
{
    ...
    for (size_t index = 0; index < system->panel_count; ++index)
    {
        ...
        DrawCylinder(cone_base, 0.05f * panel_activation, 0.0f, cone_height, 18, cone_light);
    ...
    Vector3 exit_center_3d = {origin.x, origin.y - room_radius * 0.45f, origin.z};
    Vector2 exit_center = {exit_center_3d.x, exit_center_3d.z};
